...
> **문제**: 페이지가 매 요청마다 동적으로 렌더링돼 TTFB가 1,134ms까지 걸렸어요.

> **해결**: 배포 플랫폼을 Netlify에서 Vercel로 옮기고, 동적 라우트(`[[...slug]]`)에 `force-static`을 적용해 Client Component 위주였던 페이지를 Server Component로 재구성, CDN에서 정적으로 서빙되게 했어요.

> **결과**: TTFB 1,134ms → 265ms → 약 30ms (최대 35배).
